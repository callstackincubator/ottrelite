...
Tracing of React Native internals in Java / Kotlin code (Android-only) is enabled by default if you apply the Gradle plugin. To disable it, explicitly specify, which variants should be instrumented in your Gradle build script, as shown below.
...
### Activating tracing of Java/Kotlin RN internals (Android only)
...
ottreliteTracing {
  rnInstrumentationVariants = ["debug"]
}
```
