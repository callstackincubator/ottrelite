...
Pod::Spec.new do |s|
  ...
  s.script_phase = {
    ...
    :script => <<-SCRIPT
      ...
      UMBRELLA_HEADER_PATH="${PODS_ROOT}/Headers/Public/ReactNativeOttrelite/ReactNativeOttrelite-umbrella.h"
      if [ -f "$UMBRELLA_HEADER_PATH" ]; then
        # replace all non-relative imports with commented-out lines
        sed -E '/^#import "(OttreliteOTELInterop.hpp|SpanConverter.hpp|InstrumentationScopeStore.hpp|nlohmann|google|opentelemetry|curl|openssl|brotli|zconf|zlib)[^"]*"/ s@^#import @// #import @' "$UMBRELLA_HEADER_PATH" > new_umbrella.h
